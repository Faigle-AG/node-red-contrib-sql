...
module.exports = function (RED) {
  ...
  function normalizeArrayParameters(parameters, driverName) {
    ...
    if (!Array.isArray(parameters)) {
      throw createError(
        `${driverName} parameters must be an array`,
        "INVALID_PARAMETERS",
      );
    }
    return parameters;
  ...
  function normalizeSqlServerParameters(parameters) {
    ...
    if (Array.isArray(parameters)) {
      return Object.fromEntries(
        parameters.map((value, index) => [`p${index + 1}`, value]),
      );
    }
  ...
  function SqlConfigNode(config) {
    ...
    try {
      this.port = integerOption(
        config.port,
        DEFAULT_PORTS[this.driver],
        1,
        65535,
        "Port",
      );
      this.poolMax = integerOption(
        config.poolMax,
        10,
        1,
        2147483647,
    ...
    function validateConfiguration() {
      ...
      if (!["postgres", "mysql", "mssql"].includes(node.driver)) {
        throw createError(
          `Unsupported SQL driver '${node.driver}'`,
          "UNSUPPORTED_DRIVER",
        );
      }
      if (!node.host)
        throw createError("Database host is missing", "HOST_MISSING");
      if (!node.database)
        throw createError("Database name is missing", "DATABASE_MISSING");
      if (!node.username)
    ...
    async function createPostgresPool() {
      ...
      pgPool.on("error", (err) =>
        node.error(`PostgreSQL pool error: ${err.message}`),
      );
      try {
    ...
    node.getPool = async function () {
      if (closing)
        throw createError("SQL connection is closing", "NODE_CLOSING");
      if (pool) return pool;
    ...
    node.execute = async function (statement, parameters, options = {}) {
      ...
      if (node.driver === "mysql") {
        ...
        return {
          ...
          insertId:
            result && !Array.isArray(result) ? result.insertId : undefined,

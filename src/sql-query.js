...
module.exports = function (RED) {
  ...
  function normalizeError(err, driver) {
    const wrapped = new Error(
      err && err.message ? err.message : "SQL query failed",
    );
    ...
    if (err && err.lineNumber !== undefined)
      wrapped.lineNumber = err.lineNumber;
    return wrapped;
  }
  ...
  function SqlQueryNode(config) {
    ...
    node.on("input", async function (msg, send, done) {
      ...
      try {
        if (closing)
          throw createError("SQL query node is closing", "NODE_CLOSING");
        if (!node.configNode)
          throw new Error("Missing SQL connection configuration");
        ...
        if (
          ...
          throw createError(
            "Invalid query property type",
            "INVALID_CONFIGURATION",
          );
        }
        if (typeof node.output !== "string" || !node.output.trim()) {
          throw createError(
            "Output property is missing",
            "INVALID_CONFIGURATION",
          );
        }
        const outputTarget =
        ...
        if (!closing)
          node.status.failed(String(normalized.code || normalized.message));

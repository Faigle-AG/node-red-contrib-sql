...
const editorFile =
    process.env.NODE_RED_EDITOR_JS || require.resolve('@node-red/editor-client/public/red/red.js');
...
function loadEditors() {
    ...
    vm.runInContext(
        extractFunction('validatePropertyExpression') + extractFunction('validateTypedProperty'),
        sandbox,
    );
    ...
    vm.runInContext(editorSource.slice(start, editorSource.indexOf('};;', start) + 2), sandbox);
    vm.runInContext(extractFunction('validateNodeProperty'), sandbox);
    for (const name of ['sql-config', 'sql-query']) {
        const html = fs.readFileSync(path.join(__dirname, '..', 'src', name + '.html'), 'utf8');
        vm.runInContext(
    ...
    function validate(node, key) {
        return sandbox.validateNodeProperty(node, node._def.defaults, key, node[key]);
    }
